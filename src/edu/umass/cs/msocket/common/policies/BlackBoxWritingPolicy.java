		...
		if (sockObj != null)
	      {
	        ...
	        try
	        {
	          if (sockObj.getneedToReqeustACK() )
	          {
                ...
                ByteBuffer bytebuff = ByteBuffer.wrap(bytebuf,bufferOffset,tobesent);
                ArrayList<ByteBuffer> bytebuff_list = new ArrayList<ByteBuffer>();
                bytebuff_list.add(bytebuff);
                DataMessage dm = new DataMessage(DataMessage.DATA_MESG, startSeqNum, cinfo.getDataAckSeq(), tobesent, 0, bytebuff_list,
                        bufferOffset);
